...
\label{eq:FractureVF}
\begin{align*}
    &\underbrace{\alpha_k G ((\epsilon + (1-\epsilon)(1-c^k)^2)\nabla u^k, \nabla v)}_{= \alpha_k(\partial_u J(u,c),v)}
    = 0
    \,,
    \\
    &
...
\visible<3->{
\begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth]{block title}\centering
Many choose an alternating minimization scheme (leaveing off the 3$^{rd}$ equation) due to the simplicity of the individual equations. We use Newton.
\end{beamercolorbox}}
\end{frame}
...
$R(y;x) := -\sqrt{\rho^2(x) - |y|^2} \quad y \in \mathbb R^n,~ x \in \Omega
$
%\,.
%\end{equation*}
...
\begin{equation}\label{eq:grad-lvpp}
    \aligned
 - \mathrm{div}\, \psi &-\alpha 1 + \mathrm{div}\, \varphi  &&=\; 0 \,\\
   \nabla u &-  \frac{\rho(x)}{\sqrt{1+|\psi|^2}}\psi  &&= \; 0.
    \endaligned
\end{equation}}
\pause
\only<2>{
...
%        \\
        ...
        \begin{alignedat}{3}
            ...
            &&~\fa v \in L^2(\Omega),\\
             (u, \operatorname{div} w) + \bigg(\frac{\rho\,\psi}{\sqrt{1/\alpha+|\psi|^2}}, w\bigg) 
            &= 0
