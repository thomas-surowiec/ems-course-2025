...
\begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth]{block body}
 ...
 \begin{equation*}
 	...
 	\int_\Omega f v\dd x
	+
	\underbrace{\frac{\gamma}{2}\int_{\Omega} \max\{0,\varphi - u\}^2 \dd x}_{=:\gamma \beta(u)}
 	\,.
 \end{equation*}
 Here, $f \in L^2(\Omega)$ and  $\varphi \in H^1(\Omega)$, $\varphi|_{\Gamma} \le 0$.
\end{beamercolorbox}
 ...
 \end{beamercolorbox}}
  \end{frame}
    ...
    \begin{frame}\frametitle{A Workflow for Penalty Methods}
      \visible<1->{
 \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth]{block title}\centering
Both theory and experience has shown that proving convergence in infinite dimensions (usually) leads to mesh independent numerical methods.
 \end{beamercolorbox}}\hfill   
...
%\hfill
%\begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth]{block title}\centering
%To be computationally efficient, we need to make a choice:\\ 
%Given quadrature points $x_1,\dots,x_p$, weights $w_1,\dots,w_p$, and $N_{T} = \{1,\dots,p\}$ for a cell $T$ 
%\begin{enumerate}
%\item If $u_h(x_i) - \psi_h(x_i) \le 0$, then $N_{T} := N_{T} \setminus \{i\}$
%\end{enumerate}
%\end{beamercolorbox}
\end{frame}
