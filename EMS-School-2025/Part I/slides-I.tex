...
\begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth]{block body}\centering
 ...
 \end{beamercolorbox}\vspace{-2ex}
 \visible<2->{
 \begin{minipage}{0.6\textwidth}
 \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth]{block body}
 Idea \#1: First Ritz-Galerkin: Replace $V$ by $V_{h}$. Then use a black box optimization solver, e.g., active set method. \medskip
 
 \visible<3->{ \medskip
 Problem: How do we faithfully discretize the inequality constraints?\medskip 
 }
 
  \visible<4->{ \medskip
Result: Without a non-negative basis, the discrete solution $u_{h}$ will not be globally feasible!
 }
 \end{beamercolorbox}
