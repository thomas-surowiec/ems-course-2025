...
\begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth]{block body}
 ...
 }\visible<3->{A \alert{non-negligible biactive set} is typically leads to worst-case convergence rates.\medskip
 ...
 \[
 0 = J'(u) +  \langle \lambda, G(\cdot) \rangle'(u)
 \]
...
% \end{minipage}
 ...
 \begin{frame}\frametitle{Potential Issues}
   \visible<1->{
 \begin{beamercolorbox}[rounded=true, shadow=true, wd=\textwidth]{block title}\centering
Block box optimization solvers typically use the Euclidean inner product for computing gradients. Why is that a problem?
 \end{beamercolorbox}}
...
\]
The derivative is a \alert{dual quantity}, i.e., an element of $H^{-1}(\Omega)$. }\visible<4->{But the Riesz Representation Theorem, says the \alert{gradient} $\nabla J(u)$ is the unique solution $y$ to
\[
(\nabla y, \nabla \varphi)_{L^2(\Omega)} = (\nabla u, \nabla \varphi)_{L^2(\Omega)} - (f,\varphi)_{L^2(\Omega)} \quad \varphi \in H^1_0(\Omega).
\]
...
\visible<3->{The gradient associated with the finite dimensional subspace $V_h$, however, should be
 \[
\nabla^{V_h} J_h(\mathbf{u}) := A^{-1}_h(A_h \mathbf{u} - M_h \mathbf{f}) = \mathbf{u} - A^{-1}_h \mathbf{f},
\]
